...
def niveau_center_connexion(mutations): 
   dir = "..\\.."
   niveau_centre_path = os.path.join("data_to_connect", "niveau_centre.xlsx")
...
# def add_niv_center(code_insee):
#   dir = ".."
#   niveau_centre_path = os.path.join(dir, "data_to_connect", "niveau_centre.xlsx")
#   niveau_center = pd.read_excel(niveau_centre_path, header=4)

#   niv_cent = niveau_center[niveau_center['codgeo'] == code_insee]
#   if len(niv_cent):
#     niv_cent = niv_cent.iloc[0].loc['nivcentr']
#   else:
#     niv_cent = 0
...
def pop_commune_year(mutations): 
    ...
    dir = "..\\.."
    pop_path = os.path.join("data_to_connect", "dep-com-pop/")
...
def get_dens_commune(code_insee, year):
  name_file = "insee_rp_hist.xlsx"
  pop_path = os.path.join("data_to_connect")  
  file_path = os.path.join(pop_path, name_file)
  ...
  if year > 2019:
    return df[(df['an'] == 2019) & (df['codgeo'] == code_insee)].iloc[0, :].loc['dens_pop']
  else:
    return df[(df['an'] == year) & (df['codgeo'] == code_insee)].iloc[0, :].loc['dens_pop']
...
def inflation_month(mutations, dir="drive/MyDrive/Hackathon"):
    ...
    dir = "..\\.."
    path = os.path.join("data_to_connect")
...
def get_inflation_rate(date_):
  path = os.path.join("data_to_connect", 'inflation_rate.xlsx')
  inflation_df = pd.read_excel(path)
  year = date_.year
  month = date_.strftime('%b')
  ...
  return inflation_df[inflation_df['Year'] == year].iloc[0, :][month]